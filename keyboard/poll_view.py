...
async def view_polls(message: types.Message):
    ...
    for i in range(len(chat_id)):
        ...
        n += 2
        if i == 0:
            await bot.send_message(message.from_user.id, "Дата опитування: " + date_time[i], reply_markup=get_keyboard(id[i]), reply_to_message_id=message.message_id+1)
        else:
            await bot.send_message(message.from_user.id, "Дата опитування: " + date_time[i], reply_markup=get_keyboard(id[i]), reply_to_message_id=message.message_id+n)
...
async def callbacks_command(call: types.CallbackQuery):
    await cm_start_delete_poll(call.data, call.from_user.id)
